// Adjusted from readrawdata.cc to only
// consider Relaxd data.
...
inline bool datavalid(int x, int y, int t){
	return ( (x>=0) && (x<255) && (y>=0) && (y<255) && (t>=0) && (t<12000) ) ;
}
...
class EventHeader{
	...
	public:
		...
		// If header is invalid filled -1,
		// if isvalid is true return databuf.evcount,
		// (compressed if else statement -> ?).
		...
		const char* GetDataDescription() const {
			return "evcount/I:evid/I:date/D:frame/D:errors/i" ;
		}
...
void EventHeader::Update(char* headerstr){
  ...
  // Read in the HEADER.
	// Assume it wrong till everything went fine and isvalid becomes true.
	// Otherwise return.
...
/* Directory + file name ; output file name ; number of events in file */
int readrawdata(char * relaxdfilepath, char * outrootfile, int readmaxevent ){
  // Parameters.
  ...
  if (!relaxddata.is_open()){
		...
		std::cout << "\nReading data from the file " << relaxdfilepath << "\n" ;
  ...
  if (!rootf.IsOpen()){
		...
		std::cout << "The tree will be saved in " << outrootfile << "\n" << std::endl ;

  // Make histograms.
	TH2F hpixcount("hpixcount","Integral of counts on pixels", 256, -0.5, 255.5, 256, -0.5, 255.5) ;
  TH2F hbadpix("hbadpix","Pixels with invalid values", 256, -0.5, 255.5, 256, -0.5, 255.5) ;
  ...
  //EventHeader eventheader;
	...
	// GridPix class, pointer to it because it's going to be in the tree.
  ...
  //////////////////////////////////////////
	...
	TH1D* hTime= new TH1D("hTime","time spectrum;time (clock cycles);entries",11811,0,11811) ;
  ...
  while ( (relaxddata.get(c)).good() ){
		...
		if (c == '#' && relaxddata.get(c) && c == '-'){
			...
			if (eventheader.GetCount() >= 0){
				// Repack gridpix data, due to usage of SPLIT.
				det1->RepackData() ;
				// Now that every information of the previous event is ready, fill the tree.
				tree.Fill() ;
			}
		...
		if (std::isdigit(c)){
			...
			while ( ( relaxddata >> x >> y >> t ).good() ){
				...
				std::cout << "\nRead a hit: " << x << "\t" << y << "\t" << t << std::flush ;
	      //sleep(1) ;
#endif
	      // IF VALID insert data into gridpix object 
				// (data is saved when a new header is found).
	      if (datavalid(x,y,t)){
					det1->Insert(x,y,t) ;
